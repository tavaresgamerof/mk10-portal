...
export function VideoUpload({ value, onChange, className = "", label = "Video" }: VideoUploadProps) {
  ...
  const handleFile = (file: File) => {
    ...
    if (file.size > 200 * 1024 * 1024) {
      alert("Video muito grande. Maximo 200MB.");
      return;
    }
    ...
    <div className={className}>
      ...
      {value ? (
        ...
        >
          ...
          <p className="text-text-muted/60 text-xs mt-1">MP4, WebM, OGG (max 200MB)</p>
        </div>
      )}
      <input
        ref={inputRef}
